...
export default function RecordPage() {
  ...
  const [opponentId, setOpponentId] = useState(
    opponentIdParam ? parseInt(opponentIdParam, 10) : null
  );
  ...
  const validateForm = () => {
    ...
    if (filledSets.length === 0) {
      newErrors.sets = "Enter at least one set score";
    }
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-4">
          ...
          {mode === "doubles" && (
            <div>
              <SectionTitle className="mb-2">Your partner</SectionTitle>
              <button
                type="button"
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-muted">
              Enter games per set (e.g. 6-4, 7-5). At 6-6 add tiebreak. Tiebreak-only: e.g. 12-13.
            </p>
            {errors.sets && (
